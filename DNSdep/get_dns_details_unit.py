...
def find_if_dns_third(website, ns, concentration, soa_w=None, soa_p=None):
...
def classify(website,ns,soa_w=None, soa_p=None):

    
    output = "unknown"
    concentration = read_DNS_concentration()
    output = find_if_dns_third(website, ns, concentration, soa_w,soa_p)
...
def find_and_classify(host: str) -> tuple:
    ...
    for ns in name_servers:
        ...
        ns_type[ns] = classify(host, ns, soa_w, soa_ns[ns])
        if(ns_type[ns] == "Third"):
            third+=1
    ns_groups = {}
    if(third > 0):
